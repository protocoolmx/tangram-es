...
RasterSource::RasterSource(const std::string& _name, std::unique_ptr<DataSource> _sources,
                           TileSource::SourceOptions _sourceOptions, TextureOptions _options,
                           bool _genMipmap)
    : TileSource(_name, std::move(_sources), _sourceOptions),
...
void RasterSource::clearRaster(const TileID &tileID) {
    ...
    auto rasterID = id.withMaxSourceZoom(m_sourceOptions.maxZoom);
